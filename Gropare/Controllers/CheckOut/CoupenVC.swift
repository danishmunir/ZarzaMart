...
extension CoupenVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func serverHitCanselResonList(){
        ...
        http.getRequest(urlString: Endpoints.cancelling_reasons) { (responseData) in
    ...
    func serverHitForCoupenList(){
        ...
        http.requestWithPost(parameters: dict, Url: Endpoints.couponlist) { (responseData, error) in
    ...
    func serverHitForCancelApi(){
        ...
        http.requestWithPost(parameters: dict, Url: Endpoints.delete_order) { (responseData, error) in
            DispatchQueue.main.async {
                let jsonData = responseData?.toJSONString1().data(using: .utf8)!
                let decoder = JSONDecoder()
